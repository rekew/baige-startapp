...
def get_user_by_id_endpoint (user_id: int, db: Session = Depends (get_db)):
	...
	if user is None:
		raise HTTPException (status_code = http_status.HTTP_http_status.HTTP_404_NOT_FOUND_NOT_FOUND, detail = "User not found")
	return user
...
def update_user_endpoint (
	...
	if user.phone_number:
		...
		if existing_phone and existing_phone.id != user_id:
			raise HTTPException (status_code = http_status.HTTP_http_status.HTTP_400_BAD_REQUEST_BAD_REQUEST, detail = "Phone already registered")
...
def update_user_password_endpoint (
	...
	if updated_user is None:
		...
		raise HTTPException (status_code = http_status.HTTP_400_BAD_REQUEST, detail = "Incorrect old password")
	elif updated_user == "passwords_dont_match":
		raise HTTPException (status_code = http_status.HTTP_400_BAD_REQUEST, detail = "New passwords do not match")
	return updated_user
