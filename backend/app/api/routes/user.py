...
async def get_user_by_phone_endpoint(phone: str, db: AsyncSession = Depends(get_db)):
    user_repo = UserRepository(db)
    user = await user_repo.get_by_phone_number(phone)
    if user is None:
        raise HTTPException(
            status_code=http_status.HTTP_404_NOT_FOUND, detail="User not found"
        )
...
async def update_user_endpoint(
    ...
    if user.phone_number:
        existing_phone = await user_repo.get_by_phone_number(user.phone_number)
...
async def update_user_password_endpoint(
    ...
    updated_user = await user_repo.update_password(user_id, password)
